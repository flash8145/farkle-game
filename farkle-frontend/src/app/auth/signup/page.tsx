...
export default function SignUpPage() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!passwordMatchValidation.isValid) {
      newErrors.confirmPassword = passwordMatchValidation.message || 'Passwords do not match';
    }
    ...
    try {
      ...
      if (result.success && result.token && result.user) {
        ...
        addToast({
          ...
          message: 'Welcome to Farkle! You are now signed in.'
        });
        router.push('/');
      } else {
        addToast({
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-slate-950 via-purple-950/20 to-slate-950">
        ...
        <div className="absolute top-1/2 right-1/3 w-48 h-48 bg-emerald-500/10 rounded-full blur-3xl animate-pulse delay-700" />
      </div>
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="glass-card rounded-3xl p-8 animate-bounce-in">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400">Create your account and start rolling!</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {formData.password && (
                <div className="space-y-2">
                  ...
                  <div className="text-xs text-gray-400 space-y-1">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${formData.password.length >= 8 ? 'bg-green-500' : 'bg-gray-500'}`} />
                      <span>At least 8 characters</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${/[A-Z]/.test(formData.password) ? 'bg-green-500' : 'bg-gray-500'}`} />
                      <span>One uppercase letter</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${/[0-9]/.test(formData.password) ? 'bg-green-500' : 'bg-gray-500'}`} />
            ...
            <div className="flex items-start gap-3">
              <input
                ...
                className="w-5 h-5 mt-0.5 rounded border-white/20 bg-white/5 text-purple-500 focus:ring-purple-500/20"
                required
              />
            ...
            <Button
              ...
              className="w-full btn-premium h-12 text-base font-semibold flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <>
        ...
        <div className="mt-8 text-center text-sm text-gray-500">
          <p>Join thousands of players worldwide • Safe & Secure</p>
        </div>
      </div>
    </div>
  );
